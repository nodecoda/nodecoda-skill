...
async function build() {
  ...
  if (existsSync(skillMdPath)) {
    ...
    const intro = [
      ...
      '## 为什么值得信',
      '',
      '手工搭 Dify 工作流,坑都在细节:节点 ID 和连线对不上、分支 handle 不匹配、',
      '插件依赖缺失——导入即报错,一次改动常常要重跑大半个画布。让 AI 直接生成 YAML 也一样,',
      '看着像那么回事,一导就翻车。',
      '',
      'NodeCoda 把「生成」换成「构建」:源码是结构化、可版本化的(`.ncoda` + git),',
      '每次交付都经过目标平台的完整构建校验——语法、节点、边、依赖逐项核对,',
      '过不了就不交付。所以你拿到的文件,是能直接导入 Dify 的成品,不是碰运气的草稿。',
      '',
      '边界也说清楚:校验保证「结构正确、能导入」;运行时行为(模型、工具、权限)',
      '由你在 Dify 里配置确认。',
      '',
      '## 快速上手',
      '',
